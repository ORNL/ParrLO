	...
	MPI_Barrier(lacomm);
        ...
        if(comm_rank==0)
        {
            ...
            generic.add_options()("config,c",
                po::value<std::string>(&config_file)->default_value("input.cfg"));

            // run-time options to be read from config file
            po::options_description config("Configuration");
            config.add_options()
                ("Matrix.nrows", po::value<int>()->required(), "number of matrix rows")
                ("Matrix.ncols", po::value<int>()->required(), "number of matrix columns");
